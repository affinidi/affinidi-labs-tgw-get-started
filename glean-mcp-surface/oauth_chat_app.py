...
Env vars:
  ...
  REDIRECT_URI          - OAuth redirect URI (set to your ngrok URL /callback)
...
# ── Server-side OAuth state store (avoids session-cookie domain issues with ngrok) ──
...
# ── Glean Direct config ────────────────────────────────────────────────────────
GLEAN_GATEWAY_URL = os.environ.get(
    "GLEAN_GATEWAY_URL",
    ""
)
GLEAN_CLIENT_ID = os.environ.get("GLEAN_CLIENT_ID", "")
GLEAN_CLIENT_SECRET = os.environ.get("GLEAN_CLIENT_SECRET", "")
GLEAN_REDIRECT_URI = os.environ.get(
    "GLEAN_REDIRECT_URI", f"http://localhost:{PORT}/glean/callback")
GLEAN_AUTH_URL = os.environ.get("GLEAN_AUTH_URL", "")
GLEAN_TOKEN_URL = os.environ.get("GLEAN_TOKEN_URL", "")
GLEAN_USERINFO_URL = os.environ.get("GLEAN_USERINFO_URL", "")
...
<head>
  ...
  <title>Glean Agent Gateway – Login</title>
  <style>
...
<body>
  <div class="card">
    <div class="nav">
      ...
      <a href="/glean">🔍 Glean Direct</a>
    </div>
    <div class="logo">⚡</div>
    <h1>Glean via Agent Gateway</h1>
    <p class="subtitle">The Agent Gateway is protected with <strong>Google OAuth</strong>.<br>The Glean MCP server is protected with <strong>Glean OAuth</strong> — handled by the gateway on your behalf.</p>
    ...
    <div class="flow">
      ...
      <div class="flow-node">
        <div class="flow-icon">🔐</div>
        <div class="flow-box" style="border-color:#fbbf24;background:#2d1a00;color:#fde68a">Glean<br>OAuth</div>
        <div class="flow-label">Glean OAuth ★</div>
      </div>
      <div class="flow-arrow">&rarr;</div>
      <div class="flow-node">
        <div class="flow-icon">🔍</div>
        <div class="flow-box glean">Glean<br>MCP Server</div>
        <div class="flow-label">Protected by Glean</div>
      </div>
    </div>

    <div class="gateway-badge">
      <span>Glean Agent Gateway URL</span>
      {{ gateway_url }}
    </div>
    ...
    <p class="footer">Signing in with Google grants access to the Agent Gateway (Step 1 of 2).</p>
  </div>
</body>
</html>"""
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Glean Agent Gateway</title>
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    body {
      font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif;
...
<header>
  ...
  <div class="gateway-pill" title="{{ gateway_url }}">🔗 Glean Chat</div>
...
<div class="chat-wrapper" id="chatContainer">
  <div class="welcome">
    <h2>Glean Chat</h2>
    <p>You're authenticated with Google (Step 1 ✔). Use the actions below to call the Glean MCP server via the gateway.</p>
    ...
    <div class="flow-diagram">
      ...
      <div class="fd-node">
        <div class="fd-icon">🔐</div>
        <div class="fd-box gloauth">Glean<br>OAuth</div>
        <div class="fd-label">Glean OAuth ★</div>
      ...
      <div class="fd-node">
        ...
        <div class="fd-desc">Gateway stores Glean token</div>
      </div>
      ...
      <div class="fd-node">
        <div class="fd-icon">🔍</div>
        <div class="fd-box glean">Glean<br>MCP Server</div>
        <div class="fd-label">Protected by Glean</div>
        <div class="fd-desc">Gateway calls with Glean token</div>
      </div>
    </div>
  </div>
</div>
...
<div class="input-area">
  <div class="quick-actions">
    <button class="btn-action" onclick="listTools()">📋 List Glean Tools</button>
...
function addMessage(role, content) {
  ...
  div.innerHTML = `
    ...
    <div>
      <div class="msg-label">${role === 'user' ? 'You' : role === 'assistant' ? 'Glean Chat' : ''}</div>
...
function renderPrimaryResponse(body) {
  ...
  if (chatText) {
    ...
    let html = `
      <div class="response-card">
        <div class="response-title">Glean Chat Response</div>
...
function renderCallerContext(vp) {
  try {
    ...
    if (delegations.length > 0) {
      html += `<div>`;
      html += `<div class="ctx-section-title">🔐 Credentials Delegation <span style="color:#fbbf24;font-size:9px">(Glean OAuth — Handled by Gateway)</span></div>`;
...
function renderResponse(bubble, data, requestBody) {
  ...
  if (body && body.consent_required && body.consent_required.length > 0) {
    ...
    html += `<div class="auth-card">
      <div class="auth-title">🔐 Glean OAuth Consent Required</div>
      <div class="auth-url">${escapeHtml(authUrl)}</div>
      <p style="font-size:11px;color:#94a3b8;margin-bottom:10px">The gateway needs your permission to access Glean on your behalf. Click below to authorise — the gateway will complete the OAuth flow and retry your request.</p>
      ${authUrl ? `<a href="${escapeHtml(authUrl)}" target="_blank" class="btn-open-auth">Authorise Glean Access ↗</a>` : ''}
    </div>`;
  }
...
GLEAN_LOGIN_HTML = """<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Glean Direct – Login</title>
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; background: #0f0f11; color: #e2e8f0; min-height: 100vh; display: flex; align-items: center; justify-content: center; }
    .nav { display: flex; justify-content: center; gap: 6px; margin-bottom: 24px; }
    .nav a { font-size: 12px; color: #475569; text-decoration: none; padding: 5px 14px; border: 1px solid #1e1e30; border-radius: 20px; transition: all 0.2s; }
    .nav a:hover { color: #94a3b8; border-color: #334155; }
    .nav a.active { background: #0c2a3d; color: #38bdf8; border-color: #1e3a5f; }
    .nav a:not(.active) { }
    .card { background: #1a1a2e; border: 1px solid #2d2d44; border-radius: 16px; padding: 40px; width: 480px; text-align: center; box-shadow: 0 20px 60px rgba(0,0,0,0.5); }
    .logo { width: 56px; height: 56px; background: linear-gradient(135deg, #0ea5e9, #0284c7); border-radius: 14px; display: flex; align-items: center; justify-content: center; font-size: 26px; margin: 0 auto 20px; }
    h1 { font-size: 20px; font-weight: 600; color: #f1f5f9; margin-bottom: 6px; }
    .subtitle { font-size: 13px; color: #64748b; margin-bottom: 28px; line-height: 1.6; }
    .flow { display: flex; align-items: center; justify-content: center; margin: 0 0 28px; }
    .fn { display: flex; flex-direction: column; align-items: center; gap: 4px; }
    .fb { background: #0f172a; border: 1px solid #334155; border-radius: 8px; padding: 8px 12px; font-size: 11px; font-weight: 500; color: #e2e8f0; text-align: center; min-width: 82px; line-height: 1.4; }
    .fb.you { border-color: #0ea5e9; background: #0c2a3d; color: #7dd3fc; }
    .fb.srv { border-color: #0ea5e9; background: #0c2a3d; color: #7dd3fc; }
    .fl { font-size: 9px; color: #475569; text-transform: uppercase; letter-spacing: 0.5px; }
    .fa { font-size: 16px; color: #334155; margin: 0 6px; padding-bottom: 18px; }
    .badge { background: #0f172a; border: 1px solid #1e3a5f; border-radius: 8px; padding: 10px 14px; font-size: 12px; color: #38bdf8; margin-bottom: 28px; word-break: break-all; text-align: left; }
    .badge span { color: #475569; display: block; margin-bottom: 4px; font-size: 10px; text-transform: uppercase; letter-spacing: 0.8px; }
    .btn-glean { display: flex; align-items: center; justify-content: center; gap: 10px; width: 100%; padding: 14px 20px; background: #0ea5e9; color: #fff; border: none; border-radius: 10px; font-size: 15px; font-weight: 500; cursor: pointer; text-decoration: none; transition: background 0.2s; }
    .btn-glean:hover { background: #0284c7; }
    .btn-disabled { background: #1e293b; color: #475569; cursor: not-allowed; border: 1px solid #334155; border-radius: 10px; padding: 14px 20px; font-size: 14px; }
    .footer { margin-top: 20px; font-size: 12px; color: #334155; }
    @media (max-width: 600px) {
      body { align-items: flex-start; padding: 16px; }
      .card { width: 100%; padding: 24px 18px; border-radius: 12px; }
      h1 { font-size: 18px; }
      ...
      .flow { gap: 0; }
      .fb { min-width: 60px; font-size: 10px; padding: 6px 8px; }
      .fa { font-size: 12px; margin: 0 2px; }
      .badge { font-size: 11px; margin-bottom: 20px; }
      .btn-glean, .btn-disabled { font-size: 14px; padding: 12px 16px; }
      .logo { width: 44px; height: 44px; font-size: 20px; margin-bottom: 14px; }
    }
  </style>
</head>
<body>
  <div class="card">
    <div class="nav">
      <a href="/">🔵 Google Flow</a>
      <a href="/glean" class="active">🔍 Glean Direct</a>
    </div>
    <div class="logo">🔍</div>
    <h1>Glean Direct</h1>
    <p class="subtitle">The Agent Gateway is protected with <strong>Glean OAuth</strong>.<br>Sign in with Glean — your token is sent directly as Bearer to the gateway.</p>
    <div class="flow">
      <div class="fn"><div style="font-size:16px">👤</div><div class="fb you">You<br><span style="font-size:10px">Glean Login</span></div><div class="fl">Glean OAuth</div></div>
      <div class="fa">&rarr;</div>
      <div class="fn"><div style="font-size:16px">⚡</div><div class="fb">Agent<br>Gateway</div><div class="fl">Verifies Glean token</div></div>
      <div class="fa">&rarr;</div>
      <div class="fn"><div style="font-size:16px">🔍</div><div class="fb srv">Glean<br>MCP Server</div><div class="fl">Forwards request</div></div>
    </div>
    <div class="badge"><span>Glean Agent Gateway URL</span>{{ glean_gateway_url }}</div>
    {% if has_glean_config %}
    <a href="/glean/login" class="btn-glean">🔍&nbsp; Sign in with Glean</a>
    {% else %}
    <div class="btn-disabled">⚠️ Set GLEAN_CLIENT_ID &amp; GLEAN_AUTH_URL in .env to enable</div>
    {% endif %}
    <p class="footer">Your Glean token is passed as Bearer directly to the Agent Gateway.</p>
  </div>
</body>
</html>"""

GLEAN_CHAT_HTML = """<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Glean Direct</title>
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; background: #0f0f11; color: #e2e8f0; height: 100vh; display: flex; flex-direction: column; overflow: hidden; }
    header { display: flex; align-items: center; gap: 12px; padding: 12px 20px; background: #13131f; border-bottom: 1px solid #1e1e30; flex-shrink: 0; }
    .avatar { width: 38px; height: 38px; border-radius: 50%; border: 2px solid #0ea5e9; }
    .user-info { flex: 1; }
    .user-name { font-size: 14px; font-weight: 600; color: #f1f5f9; }
    .user-email { font-size: 12px; color: #64748b; }
    .gw-pill { background: #0c2a3d; border: 1px solid #1e3a5f; border-radius: 20px; padding: 5px 12px; font-size: 11px; color: #38bdf8; max-width: 320px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
    .flow-nav { display: flex; gap: 6px; }
    .flow-nav-link { font-size: 12px; color: #475569; text-decoration: none; padding: 5px 12px; border: 1px solid #1e1e30; border-radius: 20px; white-space: nowrap; transition: all 0.2s; }
    .flow-nav-link:hover { color: #e2e8f0; border-color: #334155; }
    .flow-nav-link.active { background: #0c2a3d; color: #38bdf8; border-color: #1e3a5f; }
    .btn-logout { padding: 6px 14px; background: transparent; border: 1px solid #334155; border-radius: 8px; color: #94a3b8; font-size: 13px; cursor: pointer; text-decoration: none; }
    .btn-logout:hover { border-color: #ef4444; color: #ef4444; }
    .chat-wrapper { flex: 1; overflow-y: auto; padding: 20px; display: flex; flex-direction: column; gap: 16px; scroll-behavior: smooth; }
    .chat-wrapper::-webkit-scrollbar { width: 6px; }
    .chat-wrapper::-webkit-scrollbar-thumb { background: #2d2d44; border-radius: 3px; }
    .msg { display: flex; gap: 10px; max-width: 90%; }
    .msg.user { align-self: flex-end; flex-direction: row-reverse; }
    .msg-icon { width: 32px; height: 32px; border-radius: 8px; flex-shrink: 0; display: flex; align-items: center; justify-content: center; font-size: 16px; }
    .msg.user .msg-icon { background: #0ea5e9; }
    .msg.assistant .msg-icon { background: #1e293b; border: 1px solid #334155; }
    .msg-bubble { border-radius: 12px; padding: 12px 16px; font-size: 13px; line-height: 1.6; }
    .msg.user .msg-bubble { background: #0ea5e9; color: #fff; border-bottom-right-radius: 4px; }
    .msg.assistant .msg-bubble { background: #1a1a2e; border: 1px solid #2d2d44; border-bottom-left-radius: 4px; }
    .msg-label { font-size: 10px; color: #475569; text-transform: uppercase; letter-spacing: 0.6px; margin-bottom: 4px; }
    .msg.user .msg-label { text-align: right; }
    .json-block { background: #0f172a; border: 1px solid #1e293b; border-radius: 8px; padding: 12px; font-family: monospace; font-size: 11.5px; line-height: 1.7; overflow-x: auto; white-space: pre; color: #94a3b8; margin-top: 8px; }
    .json-block .key { color: #7dd3fc; } .json-block .string { color: #86efac; }
    .json-block .number { color: #fbbf24; } .json-block .bool { color: #f472b6; }
    .json-block .null { color: #94a3b8; }
    .status-badge { display: inline-block; padding: 2px 8px; border-radius: 4px; font-size: 10px; font-weight: 600; margin-bottom: 6px; }
    .status-badge.ok { background: #14532d; color: #86efac; }
    .status-badge.err { background: #4c1d1d; color: #fca5a5; }
    .status-badge.warn { background: #451a03; color: #fbbf24; }
    .response-card { margin-top: 10px; background: #111827; border: 1px solid #243041; border-radius: 12px; padding: 14px 16px; }
    .response-title { font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.8px; color: #7dd3fc; margin-bottom: 10px; }
    .response-text { color: #e2e8f0; font-size: 13px; line-height: 1.7; }
    .response-text p + p { margin-top: 12px; }
    .response-text a { color: #7dd3fc; text-decoration: none; }
    .response-text a:hover { text-decoration: underline; }
    .response-text strong { color: #f8fafc; }
    .response-text code { background: #0f172a; border: 1px solid #223048; border-radius: 6px; padding: 1px 6px; font-size: 12px; color: #cbd5e1; }
    .response-divider { border: 0; border-top: 1px solid #243041; margin: 12px 0; }
    .meta-list { display: grid; gap: 8px; margin-top: 8px; }
    .meta-row { display: grid; grid-template-columns: 120px 1fr; gap: 10px; align-items: start; font-size: 12px; }
    .meta-key { color: #64748b; }
    .meta-value { color: #cbd5e1; word-break: break-word; font-family: monospace; font-size: 11.5px; }
    .tools-list { display: grid; gap: 10px; margin-top: 8px; }
    .tool-card { background: #0f172a; border: 1px solid #223048; border-radius: 10px; padding: 12px; }
    .tool-name { font-weight: 700; color: #f8fafc; margin-bottom: 4px; }
    .tool-description { font-size: 12px; line-height: 1.6; color: #cbd5e1; }
    .raw-response { margin-top: 10px; border: 1px solid #243041; border-radius: 10px; background: #0b1220; overflow: hidden; }
    .raw-response summary { cursor: pointer; list-style: none; padding: 10px 12px; color: #94a3b8; font-size: 11px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.7px; }
    .raw-response summary::-webkit-details-marker { display: none; }
    .raw-response summary::after { content: 'Show'; float: right; color: #64748b; font-weight: 500; }
    .raw-response[open] summary::after { content: 'Hide'; }
    .raw-response .json-block { margin-top: 0; border: 0; border-top: 1px solid #243041; border-radius: 0; background: #0f172a; }
    .loading { display: flex; gap: 4px; align-items: center; padding: 4px 0; }
    .loading span { width: 7px; height: 7px; border-radius: 50%; background: #0ea5e9; animation: bounce 1.2s infinite; }
    .loading span:nth-child(2) { animation-delay: 0.2s; }
    .loading span:nth-child(3) { animation-delay: 0.4s; }
    @keyframes bounce { 0%,80%,100%{transform:scale(0.7);opacity:0.5} 40%{transform:scale(1);opacity:1} }
    .welcome { text-align: center; padding: 32px 20px 20px; color: #334155; }
    .welcome h2 { font-size: 18px; color: #475569; margin-bottom: 8px; }
    .welcome p { font-size: 12.5px; line-height: 1.6; }
    .input-area { padding: 12px 20px 16px; background: #13131f; border-top: 1px solid #1e1e30; flex-shrink: 0; }
    .quick-actions { display: flex; gap: 8px; margin-bottom: 10px; flex-wrap: wrap; }
    .btn-action { padding: 6px 14px; background: #1e1e30; border: 1px solid #334155; border-radius: 20px; color: #94a3b8; font-size: 12px; cursor: pointer; transition: all 0.2s; }
    .btn-action:hover { background: #0c2a3d; color: #7dd3fc; border-color: #0ea5e9; }
    .input-row { display: flex; gap: 10px; align-items: flex-end; }
    textarea#msgInput { flex: 1; background: #1a1a2e; border: 1px solid #2d2d44; border-radius: 10px; color: #e2e8f0; font-size: 13px; padding: 10px 14px; resize: none; min-height: 44px; max-height: 140px; line-height: 1.5; font-family: inherit; outline: none; transition: border-color 0.2s; }
    textarea#msgInput:focus { border-color: #0ea5e9; }
    textarea#msgInput::placeholder { color: #334155; }
    .btn-send { padding: 10px 20px; background: #0ea5e9; border: none; border-radius: 10px; color: #fff; font-size: 14px; font-weight: 500; cursor: pointer; height: 44px; transition: background 0.2s; flex-shrink: 0; }
    .btn-send:hover { background: #0284c7; }
    .btn-send:disabled { background: #0c2a3d; opacity: 0.5; cursor: not-allowed; }
    .modal-overlay { display: none; position: fixed; inset: 0; background: rgba(0,0,0,0.7); z-index: 100; align-items: center; justify-content: center; }
    .modal-overlay.open { display: flex; }
    .modal { background: #1a1a2e; border: 1px solid #2d2d44; border-radius: 16px; padding: 24px; width: 560px; max-width: 90vw; }
    .modal h3 { font-size: 16px; margin-bottom: 14px; color: #f1f5f9; }
    .modal textarea { width: 100%; background: #0f172a; border: 1px solid #334155; border-radius: 8px; color: #94a3b8; font-family: monospace; font-size: 12px; padding: 12px; height: 200px; resize: vertical; outline: none; }
    .modal-actions { display: flex; gap: 8px; margin-top: 12px; justify-content: flex-end; }
    .btn-modal-cancel { padding: 8px 16px; background: transparent; border: 1px solid #334155; border-radius: 8px; color: #94a3b8; cursor: pointer; font-size: 13px; }
    .btn-modal-send { padding: 8px 16px; background: #0ea5e9; border: none; border-radius: 8px; color: #fff; cursor: pointer; font-size: 13px; }
    @media (max-width: 600px) {
      header { padding: 8px 12px; gap: 8px; }
      .avatar { width: 30px; height: 30px; }
      .user-name { font-size: 13px; }
      .user-email { display: none; }
      .gw-pill { display: none; }
      .btn-logout { padding: 5px 10px; font-size: 12px; }
      .chat-wrapper { padding: 12px; gap: 12px; }
      .msg { max-width: 100%; }
      .json-block { font-size: 10.5px; }
...
<body>
<header>
  <img src="{{ user_picture }}" alt="avatar" class="avatar" onerror="this.src='data:image/svg+xml,<svg xmlns=%22http://www.w3.org/2000/svg%22 viewBox=%220 0 40 40%22><rect width=%2240%22 height=%2240%22 fill=%22%230ea5e9%22 rx=%2220%22/><text y=%2226%22 x=%2214%22 font-size=%2220%22 fill=%22white%22>G</text></svg>'">
  <div class="user-info">
    <div class="user-name">{{ user_name }}</div>
    <div class="user-email">{{ user_email }}</div>
  </div>
  <div class="gw-pill" title="{{ glean_gateway_url }}">🔍 Glean Chat</div>
  <a href="/glean/logout" class="btn-logout">Logout</a>
</header>

<div class="chat-wrapper" id="chatContainer">
  <div class="welcome">
    <h2>Glean Chat</h2>
    <p>Authenticated with Glean OAuth ✔. Your Glean token is sent directly to Glean Chat as Bearer.<br>The gateway verifies it and forwards your request to the Glean MCP server.</p>
  </div>
</div>
...
<script>
const GATEWAY_URL = {{ glean_gateway_url_json | safe }};
let requestId = 10;
...
function addMessage(role, content) {
  ...
  const div = document.createElement('div');
  div.className = 'msg ' + role;
  div.innerHTML = `<div class="msg-icon">${role==='user'?'👤':'🔍'}</div><div><div class="msg-label">${role==='user'?'You':'Glean Chat'}</div><div class="msg-bubble">${content}</div></div>`;
  c.appendChild(div); scrollToBottom();
  return div.querySelector('.msg-bubble');
}
function addLoading() { return addMessage('assistant','<div class="loading"><span></span><span></span><span></span></div>'); }

function highlightJson(obj) {
  return JSON.stringify(obj,null,2).replace(/("(\\u[a-zA-Z0-9]{4}|\\[^u]|[^\\"])*"(\s*:)?|\b(true|false|null)\b|-?\d+(?:\.\d*)?(?:[eE][+\-]?\d+)?)/g, m => {
    if (/^"/.test(m)) { if (/:$/.test(m)) return `<span class="key">${m}</span>`; return `<span class="string">${m}</span>`; }
...
function renderResponse(bubble, data, requestBody) {
  const bc = data.status>=200&&data.status<300?'ok':data.status===401?'warn':'err';
  let html = '';
  const primaryResponse = renderPrimaryResponse(data.body);
  if (primaryResponse) {
    html += primaryResponse;
  }
  if (data.body && Object.keys(data.body).length > 0) {
    html += renderRawRequest(requestBody);
    html += renderRawResponse(data.body);
  } else if (data.error) {
    html += `<div class="json-block" style="color:#fca5a5">${escapeHtml(data.error)}</div>`;
  }
...
async function callGateway(body, userLabel) {
  ...
  document.getElementById('sendBtn').disabled = true;
  try {
    const resp = await fetch('/api/glean', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(body)});
    renderResponse(lb, await resp.json(), body);
  } catch(err) {
    lb.innerHTML = `<span style="color:#fca5a5">Error: ${escapeHtml(err.message)}</span>`;
  } finally {
    document.getElementById('sendBtn').disabled = false; scrollToBottom();
  }
}

function buildChatToolCall(message) {
  return {jsonrpc:'2.0',id:requestId++,method:'tools/call',params:{_meta:{agentIdentity:{name:'glean-direct-client',version:'1.0.0'}},name:'chat',arguments:{message}}};
}
...
async function callChatTool(message) {
  ...
  const requestBody = buildChatToolCall(message);
  try {
    const resp = await fetch('/api/glean/chat', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({message})});
    renderResponse(lb, await resp.json(), requestBody);
  } catch(err) {
    lb.innerHTML = `<span style="color:#fca5a5">Error: ${escapeHtml(err.message)}</span>`;
  } finally {
    document.getElementById('sendBtn').disabled = false; scrollToBottom();
...
function listTools() {
  callGateway({jsonrpc:'2.0',id:requestId++,method:'tools/list',params:{_meta:{agentIdentity:{name:'glean-direct-client',version:'1.0.0'}}}}, '📋 List Tools');
}
function sendMessage() {
  const input = document.getElementById('msgInput');
  const text = input.value.trim(); if (!text) return;
  if (text.startsWith('{')) { try { const p=JSON.parse(text); input.value=''; autoResize(input); callGateway(p,text); return; } catch(_){} }
...
@app.route("/glean")
def glean_index():
    if "glean_token" in session:
        return render_template_string(
            GLEAN_CHAT_HTML,
            user_name=session.get("glean_user", {}).get("name", "Glean User"),
            user_email=session.get("glean_user", {}).get("email", ""),
            user_picture=session.get("glean_user", {}).get("picture", ""),
            glean_gateway_url=GLEAN_GATEWAY_URL,
            glean_gateway_url_json=json.dumps(GLEAN_GATEWAY_URL),
        )
    return render_template_string(
        GLEAN_LOGIN_HTML,
        glean_gateway_url=GLEAN_GATEWAY_URL,
        has_glean_config=bool(GLEAN_CLIENT_ID and GLEAN_AUTH_URL),
    )


@app.route("/glean/login")
def glean_login():
    if not GLEAN_CLIENT_ID or not GLEAN_AUTH_URL:
        return "Glean OAuth is not configured. Set GLEAN_CLIENT_ID and GLEAN_AUTH_URL in .env", 503
    state = secrets.token_urlsafe(32)
    _put_state(state)
    params = {
        "client_id": GLEAN_CLIENT_ID,
        "redirect_uri": GLEAN_REDIRECT_URI,
        "response_type": "code",
        "scope": "openid email profile",
        "state": state,
    }
    return redirect(f"{GLEAN_AUTH_URL}?{urllib.parse.urlencode(params)}")


@app.route("/glean/callback")
def glean_callback():
    error = request.args.get("error")
    if error:
        return f"Glean OAuth error: {error}", 400

    code = request.args.get("code")
    state = request.args.get("state")
    if not code or not _pop_state(state or ""):
    ...
    if not token_resp.ok:
        return f"Token exchange failed: {token_resp.text}", 502

    token_data = token_resp.json()
    print(f"[Glean] Token response keys: {list(token_data.keys())}")
    access_token = token_data.get("access_token") or token_data.get("id_token")
    if not access_token:
        return "No access token in response", 502

    print(f"[Glean] Stored token (first 60 chars): {access_token[:60]}...")
    session["glean_token"] = access_token

    # Fetch user info if endpoint is configured
    if GLEAN_USERINFO_URL:
        try:
            ui = requests.get(
                GLEAN_USERINFO_URL,
                headers={"Authorization": f"Bearer {access_token}"},
            ...
            if ui.ok:
                session["glean_user"] = ui.json()
        except Exception:
            pass

    if "glean_user" not in session:
        session["glean_user"] = {
            "name": "Glean User", "email": "", "picture": ""}

    return redirect("/glean")


@app.route("/glean/logout")
def glean_logout():
    session.pop("glean_token", None)
    session.pop("glean_user", None)
    session.pop("glean_oauth_state", None)
    return redirect("/glean")
...
@app.route("/api/glean", methods=["POST"])
def glean_gateway_request():
    if "glean_token" not in session:
        return jsonify({"error": "Not authenticated with Glean"}), 401

    body = request.get_json(silent=True)
    if not body:
        return jsonify({"error": "Invalid JSON body"}), 400

    token = session["glean_token"]
    req_headers = {
    ...
    try:
        ...
        print(f"[Glean] Request error: {e}")
        return jsonify({"error": str(e), "status": 0, "body": {}}), 502


@app.route("/api/glean/chat", methods=["POST"])
def glean_chat_request():
    if "glean_token" not in session:
        return jsonify({"error": "Not authenticated with Glean"}), 401
    ...
    req_headers = {
        "Authorization": f"Bearer {session['glean_token']}",
        "Content-Type": "application/json",
    }
    try:
        status_code, resp_body = _call_chat_tool(
            url=GLEAN_GATEWAY_URL,
            req_headers=req_headers,
            agent_name="glean-direct-client",
            message=message,
            label="Glean",
        )
        return jsonify({"status": status_code, "body": resp_body})
    except requests.exceptions.RequestException as e:
        print(f"[Glean] Chat request error: {e}")
        return jsonify({"error": str(e), "status": 0, "body": {}}), 502
...
if __name__ == "__main__":
    ...
    print(f"  Glean Direct: http://localhost:{PORT}/glean")
    print(f"  Callback:     {REDIRECT_URI}")
    print(f"  Gateway (Google): {GATEWAY_URL}")
    print(f"  Gateway (Glean):  {GLEAN_GATEWAY_URL}")
    print(f"{'='*60}\n")
    print("  Tip: set REDIRECT_URI env var to your ngrok URL /callback")
    print("       and GLEAN_REDIRECT_URI to your ngrok URL /glean/callback")
    print("       before running if using ngrok.\n")
