...
case "$EXPOSE_CHOICE" in
    ...
    2)
        ...
        if [ -z "$NGROK_URL" ]; then
            echo "Error: Could not retrieve ngrok public URL. Check /tmp/ngrok-mcp.log for details."
            exit 1
        fi
...
echo "Public MCP endpoint URL (use this in the Agent Gateway): ${PUBLIC_BASE_URL}"
echo ""
